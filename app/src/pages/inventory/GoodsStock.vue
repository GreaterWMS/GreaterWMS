<template>
  <q-list bordered padding>
      <q-item>
        <q-item-section>
          <q-item-label overline>{{ $t('notice.mobile_goodsstock.notice1') }}</q-item-label>
          <q-item-label caption>{{ $t('notice.mobile_goodsstock.notice2') }}</q-item-label>
        </q-item-section>
      </q-item>
      <q-separator spaced />
      <template v-for="(item, index) in tablelist" :key="index">
        <q-item>
          <q-item-section @click="DetailLine(item.goods_code)">
          ...
          <q-item-section side top>
            <q-item-label caption>{{ $t('notice.mobile_goodsstock.notice3') }}:{{ item.onhand_stock }}</q-item-label>
            <q-item-label caption>{{ $t('notice.mobile_goodsstock.notice4') }}{{ item.can_order_stock }}</q-item-label>
          </q-item-section>
        </q-item>
...
export default defineComponent({
  ...
  setup () {
    ...
    function DetailLine(e) {
      ...
      oldlink.value = 'goodsstock'
      newlink.value = 'binstock'
      $router.push({ name: 'binstock' })
    }
