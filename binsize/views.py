...
class API(APIView):
    '''
        ...
        **获取数据列表**<br><br>
        ...
        |   binl   |  float    |  长(单位：mm)  |  N   |  接收一个float类型的数字 |
        |   binw   |  float    |  宽(单位：mm)  |  N   |  接收一个float类型的数字 |
        |   binh   |  float    |  高(单位：mm)  |  N   |  接收一个float类型的数字 |
        ...
        **一次只能创建一条数据**<br><br>
        ...
        |   binl   |   float   | 长(单位：mm) |    Y |  接收任意一个float数字,单位为毫米   |
        |   binw   |   float   | 宽(单位：mm) |    Y |  接收任意一个float数字,单位为毫米   |
        |   binh   |   float   | 高(单位：mm) |    Y |  接收任意一个float数字,单位为毫米   |
            ...
            if request._request.GET.get('getfile', ''):
                if str(request._request.GET.get('getfile', '')) == "1":
                    ...
                    for i in range(len(file_data)):
                        ...
                        file_detail2.append(file_data[i].binl)
                        file_detail3.append(file_data[i].binw)
                        file_detail4.append(file_data[i].binh)
                    ...
                    df = pd.DataFrame({pathname: file_detail1,
                                       '长(单位：mm)': file_detail2,
                                       '宽(单位：mm)': file_detail3,
                                       '高(单位：mm)': file_detail4,
                                       '创建时间': file_detail5,
                                       '最后更新时间': file_detail6})
                ...
                if request._request.GET.get('binl', ''):
                    list = list.filter(binl=float(request._request.GET.get('binl', ''))).order_by(sort)
                if request._request.GET.get('binw', ''):
                    list = list.filter(binl=float(request._request.GET.get('binw', ''))).order_by(sort)
                if request._request.GET.get('binh', ''):
                    list = list.filter(binl=float(request._request.GET.get('binh', ''))).order_by(sort)
            ...
            try:
                ...
                if ListModel.objects.filter(appid=request.user.appid, name=str(data['name']), is_delete=0).exists():
                    ...
                    ListModel.objects.create(appid=request.user.appid, name=str(data['name']), create_name=create_name,
                                             binl=round(float(data['binl']), 2), binw=round(float(data['binw']), 2),
                                             binh=round(float(data['binh']), 2), t_code=Md5.md5(str(data['name'])))
                    ip = request.META.get('HTTP_X_FORWARDED_FOR') if request.META.get(
                        'HTTP_X_FORWARDED_FOR') else request.META.get('REMOTE_ADDR')
                    ret = FBMsg.wms_ret()
                    ret['ip'] = ip
        ...
        if vip_check == "N":
            ...
            try:
                ...
                if ListModel.objects.filter(appid=request.user.appid, t_code=str(data['t_code']), is_delete=0).exists():
                    ...
                    if 'binl' in data:
                        patch_data.binl = round(float(data['binl']), 2)
                    if 'binw' in data:
                        patch_data.binw = round(float(data['binw']), 2)
                    if 'binh' in data:
                        patch_data.binh = round(float(data['binh']), 2)
                    patch_data.save()
                    ip = request.META.get('HTTP_X_FORWARDED_FOR') if request.META.get(
                        'HTTP_X_FORWARDED_FOR') else request.META.get('REMOTE_ADDR')
                    ret = FBMsg.wms_ret()
                ...
                if str(row[pathname]) == '':
                    ...
                    if ListModel.objects.filter(appid=request.user.appid, name=str(row[pathname]), is_delete=0).exists():
                        ...
                        ListModel.objects.create(appid=request.user.appid, name=str(row[pathname]),
                                                 ...
                                                 binl=round(float(row['长(单位：mm)']), 2), binw=round(float(row['宽(单位：mm)']), 2),
                                                 binh=round(float(row['高(单位：mm)']), 2), t_code=Md5.md5(str(row[pathname])))
            os.remove(filename)
            ip = request.META.get('HTTP_X_FORWARDED_FOR') if request.META.get(
                'HTTP_X_FORWARDED_FOR') else request.META.get('REMOTE_ADDR')
            ret = FBMsg.wms_ret()
