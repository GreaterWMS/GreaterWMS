...
class AsnDetailViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if AsnListModel.objects.filter(openid=self.request.auth.openid, asn_code=str(data['asn_code']),
            ...
            raise APIException({"detail": "ASN Code has been deliveried or does not exists"})
...
class AsnSortedViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, pk):
        qs = self.get_object()
        if qs.asn_status != 3:
            raise APIException({"detail": "This ASN Status does not correct"})
        else:
            data = request.data
            for i in range(len(data['goods_code'])):
                check_data = {
            ...
            for j in range(len(data['goods_code'])):
                ...
                asn_detail.goods_actual_qty = int(data['goods_qty'][j])
                goods_qty_check = asn_detail.goods_qty - int(data['goods_qty'][j])
...
class MoveToBinViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, pk):
        ...
        if qs.openid != self.request.auth.openid:
            ...
            if qs.asn_status != 4:
                raise APIException({"detail": "ASN Status does not correct , Can not Move To Bin"})
            else:
                data = request.data
