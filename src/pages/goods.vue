    ...
    >
      <div class="row no-wrap q-pa-md">
        <div class="column">
          ...
          <q-btn label="现有库存为你现在进出库接触后的即时库存，只接受整数" size="md" align="left" flat icon="warning"/>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'on_hand_stock', label: '现有库存', field: 'on_hand_stock' },
  ...
  methods: {
    getList () {
      ...
      get('goods/list/', { openid: openid, page: this.current, max_page: 50 }).then(response => {
    ...
    downloadexample () {
      openURL('https://scmapi.56yhz.com/goods/ex/')
    }
  },
  created () {
    this.uploaded = 'https://scmapi.56yhz.com/goods/?openid=' + this.$q.localStorage.getItem('openid')
  },
  mounted () {
