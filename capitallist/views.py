            ...
            if request._request.GET.get('getfile', ''):
                if str(request._request.GET.get('getfile', '')) == "1":
                    ...
                    for i in range(len(file_data)):
                        ...
                        file_detail2.append(file_data[i].qty)
                        file_detail3.append(file_data[i].amount)
                        file_detail4.append(file_data[i].create_name)
                ...
                if request._request.GET.get('qty', ''):
                    list = list.filter(qty=int(request._request.GET.get('qty', ''))).order_by(sort)
                if request._request.GET.get('amount', ''):
                    list = list.filter(amount=round(float(request._request.GET.get('amount', '')), 2)).order_by(sort)
            ...
            try:
                ...
                if ListModel.objects.filter(appid=request.user.appid, name=str(data['name']), is_delete=0).exists():
                    ...
                    ListModel.objects.create(appid=request.user.appid, name=str(data['name']), create_name=create_name,
                                             qty=int(data['qty']), amount=round(float(data['amount']), 2),
        ...
        if vip_check == "N":
            ...
            try:
                ...
                if ListModel.objects.filter(appid=request.user.appid, t_code=str(data['t_code']), is_delete=0).exists():
                    ...
                    if 'qty' in data:
                        patch_data.qty = int(data['qty'])
                    if 'amount' in data:
                        patch_data.amount = round(float(data['amount']), 2)
                    patch_data.save()
                    ip = request.META.get('HTTP_X_FORWARDED_FOR') if request.META.get(
                        'HTTP_X_FORWARDED_FOR') else request.META.get('REMOTE_ADDR')
                    ret = FBMsg.wms_ret()
