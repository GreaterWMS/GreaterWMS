            ...
            if request._request.GET.get('getfile', ''):
                if str(request._request.GET.get('getfile', '')) == "1":
                    ...
                    for i in range(len(file_data)):
                        ...
                        file_detail2.append(file_data[i].property_code)
                ...
                if request._request.GET.get('property_code', ''):
                    list = list.filter(property_code=int(request._request.GET.get('property_code', ''))).order_by(sort)
        ...
        if vip_check == "N":
            ...
            try:
                ...
                if ListModel.objects.filter(appid=request.user.appid, name=str(data['name']), is_delete=0).exists():
                    ...
                    ListModel.objects.create(appid=request.user.appid, name=data['name'], create_name=create_name,
                                             property_code=int(data['property_code']), t_code=Md5.md5(data['name']))
                    ip = request.META.get('HTTP_X_FORWARDED_FOR') if request.META.get(
                        'HTTP_X_FORWARDED_FOR') else request.META.get('REMOTE_ADDR')
                    ret = FBMsg.wms_ret()
                    ret['ip'] = ip
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if vip_check == "N":
            ...
            try:
                ...
                if ListModel.objects.filter(appid=request.user.appid, t_code=data['t_code'], is_delete=0).exists():
                    patch_name = ListModel.objects.get(t_code=data['t_code']).name
                    if patch_name != data['name']:
                        if ListModel.objects.filter(appid=request.user.appid, name=data['name'], is_delete=0).exists():
                            ret = FBMsg.wms_same()
                            ret['data'] = data
                            return Response(ret)
                    patch_data = ListModel.objects.filter(appid=request.user.appid, t_code=data['t_code'], is_delete=0).first()
                    if 'name' in data:
                        patch_data.name = data['name']
                    if 'property_code' in data:
                        patch_data.property_code = int(data['property_code'])
                    patch_data.save()
                    ip = request.META.get('HTTP_X_FORWARDED_FOR') if request.META.get(
                        'HTTP_X_FORWARDED_FOR') else request.META.get('REMOTE_ADDR')
                    ret = FBMsg.wms_ret()
            ...
            for index, row in df.iterrows():
                if str(row[pathname]) == '':
                    ...
                    if ListModel.objects.filter(appid=request.user.appid, name=str(row[pathname]), is_delete=0).exists():
                        ...
                        ListModel.objects.create(appid=request.user.appid, name=str(row[pathname]), property_code=int(row['属性权限']),
                                                 create_name=request.user.nickname, t_code=Md5.md5(str(row[pathname])))
            os.remove(filename)
            ip = request.META.get('HTTP_X_FORWARDED_FOR') if request.META.get(
                'HTTP_X_FORWARDED_FOR') else request.META.get('REMOTE_ADDR')
            ret = FBMsg.wms_ret()
