...
class APIViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = serializers.WarehouseGetSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        if self.queryset.filter(openid=data['openid'], warehouse_name=data['warehouse_name'], is_delete=False).exists():
            raise APIException({"detail": "Data Exists"})
        else:
            if self.queryset.filter(openid=data['openid'], is_delete=False).count() >= 1:
