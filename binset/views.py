            ...
            if request._request.GET.get('getfile', ''):
                if str(request._request.GET.get('getfile', '')) == "1":
                    ...
                    for i in range(len(file_data)):
                        ...
                        file_detail2.append(file_data[i].bin_size)
                        file_detail3.append(file_data[i].bin_property)
                        file_detail4.append(file_data[i].bin_empty)
                        file_detail5.append(file_data[i].create_name)
                        file_detail6.append(file_data[i].create_time.strftime("%Y-%m-%d %H:%M:%S"))
                        file_detail7.append(file_data[i].last_update_time.strftime("%Y-%m-%d %H:%M:%S"))
                    print("库位名称：", file_detail1)
                    print("库位尺寸：", file_detail2)
                    print("库位属性：", file_detail3)
                    print("空库位：", file_detail4)
                    print("创建时间：", file_detail5)
                    print("最后更新时间：", file_detail6)
                ...
                if request._request.GET.get('bin_empty', ''):
                    list = list.filter(bin_empty=int(request._request.GET.get('bin_empty', ''))).order_by(sort)
                ...
                bin_sizelist = []
                bin_propertylist = []
                bin_size_data = binsize.models.ListModel.objects.filter(appid=request.user.appid, is_delete=0)
                bin_property_data = property.models.ListModel.objects.filter(appid=request.user.appid, is_delete=0)
                for i in range(len(bin_size_data)):
                    bin_sizelist.append(bin_size_data[i].name)
                for j in range(len(bin_property_data)):
                    bin_propertylist.append(bin_property_data[j].name)
                ret = FBMsg.ret()
                ret['ip'] = ip
                ret['data'] = list_ser.data
                ret['totlepage'] = math.ceil(list.count()/int(max_page))
                ret['bin_sizelist'] = bin_sizelist
                ret['bin_propertylist'] = bin_propertylist
            ...
            try:
                ...
                if ListModel.objects.filter(appid=request.user.appid, t_code=str(data['t_code']), is_delete=0).exists():
                    ...
                    if patch_name != str(data['name']):
                        if ListModel.objects.filter(appid=request.user.appid, name=str(data['name']),
                                                    is_delete=0).exists():
                            ret = FBMsg.wms_same()
                            ret['data'] = data
                            return Response(ret)
                    ...
                    if 'bin_size' in data:
                        patch_data.bin_size = str(data['bin_size'])
                    if 'bin_property' in data:
                        patch_data.bin_property = str(data['bin_property'])
                    patch_data.save()
                    ip = request.META.get('HTTP_X_FORWARDED_FOR') if request.META.get(
                        'HTTP_X_FORWARDED_FOR') else request.META.get('REMOTE_ADDR')
                    ret = FBMsg.wms_ret()
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if vip_check == "N":
            ...
            for i in range(len(data)):
                if ListModel.objects.filter(appid=request.user.appid, t_code=str(data[i]['t_code']),
                                            is_delete=0).exists():
                    pass
                else:
                    ret = FBMsg.wms_err()
            ...
            for j in range(len(data)):
                delete_data = ListModel.objects.filter(appid=request.user.appid, t_code=str(data[j]['t_code']),
                                                       is_delete=0).first()
