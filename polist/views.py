...
from .serializers import ListSerializers, GoodsListSerializers, SupplierListSerializers
from django.conf import settings
import os, math, datetime, goodslist, supplier, stocklist
import pandas as pd
    ...
    def get(self, request, *args, **kwargs):
        ...
        if vip_check == "N":
            ...
            if request._request.GET.get('getfile', ''):
                ...
                delete_list = delete_all_list.filter(po_status=1)
                ...
                supplier_list = supplier.models.ListModel.objects.filter(appid=request.user.appid, is_delete=0).order_by('name')
                supplier_list_ser = SupplierListSerializers(supplier_list, many=True)
                ret = FBMsg.ret()
                ret['ip'] = ip
                ret['data'] = list_ser.data
                ret['totlepage'] = math.ceil(list.count()/int(max_page))
                ret['goods_code_list'] = goods_code_list_ser.data
                ret['supplier_list'] = supplier_list_ser.data
        ...
        if vip_check == "N":
            ...
            for i in range(len(data)):
                if ListModel.objects.filter(appid=request.user.appid, t_code=data[i]['t_code'], is_delete=0).exists():
                    delete_status = ListModel.objects.filter(appid=request.user.appid, t_code=data[i]['t_code'], is_delete=0).first().po_status
                    if delete_status != 1:
                        ret = FBMsg.wms_po_status_1()
                        ret['data'] = data
                        return Response(ret)
                else:
                    ret = FBMsg.wms_err()
