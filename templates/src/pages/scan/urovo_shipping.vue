<template>
  <q-page>
    <q-card v-show="!fab" class="shadow-24" :style="{ width: width, height: height }">
      <q-card-section>
        <q-bar class="bg-white shadow-1 q-mb-sm" style="font-size: 12px;">{{ $t('scan.view_shipping.shipping_code') }}: {{ goods_scan }}</q-bar>
        <q-btn-group push>
      ...
      <q-separator dark />
      <q-bar style="font-size: 12px;background: #D7D7D7;">{{ $t('scan.view_shipping.driver_info') }}</q-bar>
      <div class="q-px-md q-py-sm">
        <q-bar class="bg-white" style="justify-content: space-between; padding: 0;">
          <div class="text-center" style="height: 40px;line-height: 40px; width: 60px;font-size: 12px;">
            {{ $t('scan.view_shipping.license_plate_number') }}:
          </div>
          <q-select style="width: 70%;" dense outlined hide-dropdown-icon v-model="carNumber" :options="options1" use-input />
        </q-bar>
        <q-bar class="bg-white q-my-md" style="justify-content: space-between;padding: 0;">
          <div class="text-center" style="height: 40px;line-height: 40px;width: 60px;font-size: 12px;">
            {{ $t('scan.view_shipping.name') }}:
          </div>
          <q-select style="width: 70%;" dense outlined hide-dropdown-icon v-model="name" :options="options2" use-input />
        </q-bar>
        <q-bar class="bg-white" style="justify-content: space-between;padding: 0;">
          <div class="text-center" style="height: 40px;line-height: 40px;width: 60px;font-size: 12px;">
            {{ $t('scan.view_shipping.contact_info') }}:
          </div>
          <q-select style="width: 70%;" dense outlined hide-dropdown-icon v-model="contactInfo" :options="options3" use-input />
        </q-bar>
      </div>
    </q-card>
...
export default {
  ...
  data() {
    return {
      ...
      options2: [],
      options3: [],
      error: this.$t('scan.scan_goods_label_error')
    };
  },
  methods: {
    keyboardHandler(){
    ...
    datachange() {
      ...
      getauth('scanner/?bar_code=' + _this.barscan, {})
        .then(res => {
          ...
          } else if (res.results[0].mode === 'GOODS' && _this.barscan) {
            _this.goods_scan = res.results[0].code;
            _this.bin_scan = '';
            _this.countAdd(_this.goods_scan);
            _this.showBg = true;
          } else if (res.results[0].mode === 'BINSET' && _this.barscan) {
    ...
    countAdd(e) {
      ...
      _this.table_list.filter(function(value, index, array) {
        if (value.bin_name === _this.bin_scan && value.goods_code === e) {
          ...
          if (_this.table_list[index].physical_inventory > _this.table_list[index]) {
            _this.$q.notify({
              message: error2,
              icon: 'close',
              color: 'negative'
  ...
  mounted() {
    ...
    window.addEventListener('focusin',_this.keyboardHandler)
    _this.width = _this.$q.screen.width * 1 + '' + 'px';
    _this.height = _this.$q.screen.height - 50 + '' + 'px';
    _this.scroll_height = _this.$q.screen.height - 335 + '' + 'px';
    _this.barscan = '';
    _this.bin_scan = '';
    _this.goods_scan = '';
    _this.scanEvents();
  ...
  beforeDestroy() {
    ...
    window.removeEventListener('focusin', _this.keyboardHandler, false);
  },
  destroyed() {}
};
</script>
