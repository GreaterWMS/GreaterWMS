      ...
      >
         ...
         <template v-slot:body="props">
           <q-tr :props="props">
             ...
             <template v-else-if="editMode" style="width: 100px">
               <template v-if="props.row.id === editid">
...
export default {
  ...
  data () {
    return {
      ...
      table_list: [],
      columns: [
        { name: 'goods_class', required: true, label: this.$t('goods.view_class.goods_class'), align: 'left', field: 'goods_class' },
  ...
  methods: {
    ...
    getSearchList () {
      ...
      if (_this.$q.localStorage.has('auth')) {
        ...
        getauth(_this.pathname + '?goods_class__icontains=' + _this.filter + '&page=' + '' + _this.current, {
        }).then(res => {
          _this.table_list = res.results
          _this.total = res.count
          if (res.count === 0) {
    ...
    newDataSubmit () {
      ...
      if (goodsclasses.indexOf(_this.newFormData.goods_class) === -1 && _this.newFormData.goods_class.length !== 0) {
      ...
      } else if (goodsclasses.indexOf(_this.newFormData.goods_class) !== -1) {
        _this.$q.notify({
          message: _this.$t('notice.goodserror.goods_classerror'),
          icon: 'close',
          color: 'negative'
        })
      } else if (_this.newFormData.goods_class.length === 0) {
        _this.$q.notify({
