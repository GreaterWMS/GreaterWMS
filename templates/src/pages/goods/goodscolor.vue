...
export default {
  ...
  data () {
    return {
      ...
      table_list: [],
      columns: [
        { name: 'goods_color', required: true, label: this.$t('goods.view_color.goods_color'), align: 'left', field: 'goods_color' },
      ...
      deleteForm: false,
      deleteid: 0,
      error1: this.$t('goods.view_color.error1'),
      current: 1,
      max: 0,
      total: 0,
      paginationIpt: 1
  ...
  methods: {
    ...
    getSearchList () {
      ...
      if (_this.$q.localStorage.has('auth')) {
        ...
        getauth(_this.pathname + '?goods_color__icontains=' + _this.filter + '&page=' + '' + _this.current, {
        }).then(res => {
          _this.table_list = res.results
          _this.total = res.count
          if (res.count === 0) {
