         ...
         <template v-slot:body="props">
           <q-tr :props="props">
             ...
             <template v-else-if="editMode">
               <template v-if="props.row.id === editid">
                 <q-td key="action" :props="props" style="width: 100px">
                 <q-btn round flat push color="purple" icon="check" @click="editDataSubmit()">
                   <q-tooltip content-class="bg-amber text-black shadow-4" :offset="[10, 10]" content-style="font-size: 12px">
                    {{ $t('confirmedit') }}
                  </q-tooltip>
                 </q-btn>
                 <q-btn round flat push color="dark" icon="close" @click="editDataCancel()">
                   <q-tooltip content-class="bg-amber text-black shadow-4" :offset="[10, 10]" content-style="font-size: 12px">
                    {{ $t('canceledit') }}
                  </q-tooltip>
                 </q-btn>
...
export default {
  ...
  data () {
    return {
      ...
      table_list: [],
      columns: [
        { name: 'goods_unit', required: true, label: this.$t('goods.view_unit.goods_unit'), align: 'left', field: 'goods_unit' },
      ...
      deleteForm: false,
      deleteid: 0,
      error1: this.$t('goods.view_unit.error1'),
      current: 1,
      max: 0,
      total: 0,
      paginationIpt: 1
  ...
  methods: {
    ...
    getSearchList () {
      ...
      if (_this.$q.localStorage.has('auth')) {
        ...
        getauth(_this.pathname + '?goods_unit__icontains=' + _this.filter + '&page=' + '' + _this.current, {
        }).then(res => {
          _this.table_list = res.results
          _this.total = res.count
          if (res.count === 0) {
    ...
    newDataSubmit () {
      ...
      _this.table_list.forEach(i => {
        goodsunits.push(i.goods_unit)
      })
      if (goodsunits.indexOf(_this.newFormData.goods_unit) === -1 && _this.newFormData.goods_unit.length !== 0) {
      ...
      } else if (goodsunits.indexOf(_this.newFormData.goods_unit) !== -1) {
        _this.$q.notify({
          message: _this.$t('notice.goodserror.goods_uniterror'),
          icon: 'close',
          color: 'negative'
        })
      } else if (_this.newFormData.goods_unit.length === 0) {
        _this.$q.notify({
          message: _this.$t('goods.view_unit.error1'),
          icon: 'close',
          color: 'negative'
        })
      }
