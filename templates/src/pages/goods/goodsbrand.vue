...
export default {
  ...
  data () {
    return {
      ...
      table_list: [],
      columns: [
        { name: 'goods_brand', required: true, label: this.$t('goods.view_brand.goods_brand'), align: 'left', field: 'goods_brand' },
      ...
      deleteForm: false,
      deleteid: 0,
      error1: this.$t('goods.view_brand.error1'),
      current: 1,
      max: 0,
      total: 0,
      paginationIpt: 1
  ...
  methods: {
    ...
    getSearchList () {
      ...
      if (_this.$q.localStorage.has('auth')) {
        ...
        getauth(_this.pathname + '?goods_brand__icontains=' + _this.filter + '&page=' + '' + _this.current, {
        }).then(res => {
          _this.table_list = res.results
          _this.total = res.count
          if (res.count === 0) {
    ...
    newDataSubmit () {
      ...
      _this.table_list.forEach(i => {
        goodsbrands.push(i.goods_brand)
      })
      if (goodsbrands.indexOf(_this.newFormData.goods_brand) === -1 && _this.newFormData.goods_brand.length !== 0) {
      ...
      } else if (goodsbrands.indexOf(_this.newFormData.goods_brand) !== -1) {
        _this.$q.notify({
          message: _this.$t('notice.goodserror.goods_branderror'),
          icon: 'close',
          color: 'negative'
        })
      } else if (_this.newFormData.goods_brand.length === 0) {
        _this.$q.notify({
          message: _this.$t('goods.view_brand.error1'),
          icon: 'close',
          color: 'negative'
        })
      }
