...
export default {
  name: 'Pagegoodsorigin',
  data () {
    return {
      openid: '',
      login_name: '',
      authin: '0',
      pathname: 'goodsorigin/',
      pathname_previous: '',
      pathname_next: '',
      separator: 'cell',
      loading: false,
      ...
      table_list: [],
      columns: [
        { name: 'goods_origin', required: true, label: this.$t('goods.view_origin.goods_origin'), align: 'left', field: 'goods_origin' },
      ...
      deleteForm: false,
      deleteid: 0,
      error1: this.$t('goods.view_origin.error1'),
      current: 1,
      max: 0,
      total: 0,
      paginationIpt: 1
  ...
  methods: {
    ...
    getSearchList () {
      ...
      if (_this.$q.localStorage.has('auth')) {
        ...
        getauth(_this.pathname + '?goods_origin__icontains=' + _this.filter + '&page=' + '' + _this.current, {
        }).then(res => {
          _this.table_list = res.results
          _this.total = res.count
          if (res.count === 0) {
    ...
    newDataSubmit () {
      ...
      _this.table_list.forEach(i => {
        goodsorigins.push(i.goods_origin)
      })
      if (goodsorigins.indexOf(_this.newFormData.goods_origin) === -1 && _this.newFormData.goods_origin.length !== 0) {
      ...
      } else if (goodsorigins.indexOf(_this.newFormData.goods_origin) !== -1) {
        _this.$q.notify({
          message: _this.$t('notice.goodserror.goods_originerror'),
          icon: 'close',
          color: 'negative'
        })
      } else if (_this.newFormData.goods_origin.length === 0) {
        _this.$q.notify({
          message: _this.$t('goods.view_origin.error1'),
          icon: 'close',
          color: 'negative'
        })
      }
