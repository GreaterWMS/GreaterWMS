  ...
  methods: {
    getList () {
      ...
      if (_this.$q.localStorage.has('auth')) {
        getauth(_this.pathname + '&supplier=' + _this.login_name + '&page=' + '' + _this.current, {
        }).then(res => {
    ...
    getSearchList () {
      ...
      if (_this.$q.localStorage.has('auth')) {
        ...
        getauth(_this.pathname + '&supplier=' + _this.login_name + '&asn_code__icontains=' + _this.filter + '&page=' + '' + _this.current, {
        }).then(res => {
          _this.table_list = res.results
          _this.total = res.count
          if (res.count === 0) {
