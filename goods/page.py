...
class MyPageNumberPagination(PageNumberPagination):
    ...
    def get_paginated_response(self, data):
        goods_unit_list_data = goods_unit.objects.filter(Q(openid=self.request.auth.openid, is_delete=False) |
        ...
        goods_color_list_data = goods_color.objects.filter(Q(openid=self.request.auth.openid, is_delete=False) |
                                                            Q(openid='init_data', is_delete=False))
        goods_color_list = []
        for i in range(len(goods_color_list_data)):
            goods_color_list.append(goods_color_list_data[i].goods_color)
        goods_shape_list_data = goods_shape.objects.filter(Q(openid=self.request.auth.openid, is_delete=False) |
                                                            Q(openid='init_data', is_delete=False))
        goods_shape_list = []
