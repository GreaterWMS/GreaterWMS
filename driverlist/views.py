...
class API(APIView):
    '''
        ...
        **获取数据列表**<br><br>
        ...
        |   qty   |   int   | 数量 |    N |   结果为模糊查询  |
        |   amount   |   float   | 金额 |    N |   结果为模糊查询  |
        ...
        **一次只能创建一条数据**<br><br>
        ...
        |   qty   |   int   | 数量 |    Y |  资产数量为int数字   |
        |   amout   |   float   | 资产金额 |    Y |  资产金额为float类型   |
        ...
        **一次只能修改一条数据**<br><br>
        ...
        |   qty   |   int   | 数量 |    N |  资产数量为int数字   |
        |   amout   |   float   | 资产金额 |    N |  资产金额为float类型   |
            ...
            if request._request.GET.get('getfile', ''):
                if str(request._request.GET.get('getfile', '')) == "1":
                    ...
                    for i in range(len(file_data)):
                        ...
                        file_detail2.append(file_data[i].plate_license)
                        file_detail3.append(file_data[i].mobile)
                        file_detail4.append(file_data[i].identity_card)
                        file_detail5.append(file_data[i].driver_license)
                        file_detail6.append(file_data[i].vehicle_license)
                        file_detail7.append(file_data[i].create_name)
                ...
                if request._request.GET.get('plate_license', ''):
                    list = list.filter(plate_license__icontains=request._request.GET.get('plate_license', '')).order_by(sort)
                if request._request.GET.get('mobile', ''):
                    list = list.filter(mobile__icontains=int(request._request.GET.get('mobile', ''))).order_by(sort)
                if request._request.GET.get('identity_card', ''):
                    list = list.filter(identity_card__icontains=request._request.GET.get('identity_card', '')).order_by(sort)
                if request._request.GET.get('driver_license', ''):
                    list = list.filter(driver_license__icontains=request._request.GET.get('driver_license', '')).order_by(sort)
                if request._request.GET.get('vehicle_license', ''):
                    list = list.filter(vehicle_license__icontains=request._request.GET.get('vehicle_license', '')).order_by(sort)
                if request._request.GET.get('create_name', ''):
                    list = list.filter(create_name=str(request._request.GET.get('create_name', ''))).order_by(sort)
        ...
        if vip_check == "N":
            ...
            try:
                ...
                if ListModel.objects.filter(appid=request.user.appid, t_code=str(data['t_code']), is_delete=0).exists():
                    ...
                    if 'plate_license' in data:
                        patch_data.plate_license = str(data['plate_license'])
                    if 'mobile' in data:
                        patch_data.mobile = int(data['mobile'])
                    if 'identity_card' in data:
                        patch_data.identity_card = str(data['identity_card'])
                    if 'driver_license' in data:
                        patch_data.driver_license = str(data['driver_license'])
                    if 'vehicle_license' in data:
                        patch_data.vehicle_license = str(data['vehicle_license'])
                    patch_data.save()
                    ip = request.META.get('HTTP_X_FORWARDED_FOR') if request.META.get(
                        'HTTP_X_FORWARDED_FOR') else request.META.get('REMOTE_ADDR')
                    ret = FBMsg.wms_ret()
            ...
            for index, row in df.iterrows():
                if str(row[pathname]) == '':
                    ...
                    if ListModel.objects.filter(appid=request.user.appid, name=str(row[pathname]), is_delete=0).exists():
                        ...
                        ListModel.objects.create(appid=request.user.appid, name=str(row[pathname]), plate_license=str(row['车牌号']),
                                                 mobile=int(row['电话']), identity_card=str(row['身份证']),
                                                 driver_license=str(row['驾驶证编号']),
                                                 vehicle_license=str(row['行驶证编号']), create_name=request.user.name,
